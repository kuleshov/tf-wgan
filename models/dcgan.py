...
class DCGAN(object):
  ...
  def __init__(self, n_dim, n_chan=1, opt_alg='adam', opt_params=default_opt):
    ...
    with tf.name_scope('generator'):
      ...
      x = Deconvolution2D(64, 5, 5, output_shape=(128, 64, 14, 14), 
            border_mode='same', activation='relu', subsample=(2,2), 
            init='glorot_uniform', dim_ordering='th')(x)
    ...
    self.p_real = tf.reduce_mean(tf.round(d_real))
    self.p_fake = tf.reduce_mean(tf.round(d_fake))
  ...
  def gen(self, noise):
    ...
    return self.sess.run(self.P, feed_dict={X_g_in : noise})
